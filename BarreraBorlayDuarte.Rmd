...
# Preámbulos y Parámetros

## Preámbulos

Cargamos las herramientas que vamos a usar y fijamos la semilla 42 para garantizar el mismo proceso pseudo aleatorio en todas las sucesivas ejecuciones. 

```{r Carga de Liberías y Datos}
library('tidyverse')
library('stats')
set.seed(42)
```
...
## Parámetros
  ...
  Para darle reutilizabilidad a nuestro trabajo definimos todos los procedimientos como funciones. Acá están los parámetros que permitirían realizar una simulación parecida cambiando por ejemplo el proceso generador de datos o la distribución del error $\varepsilon$.

```{r Parámetros de esta Simulación}
...
#El valor de B en la consigna:
n_sims <- 1000
```

  Vamos a comparar coeficientes estimados por diversos métodos con los de un proceso generador de datos conocido, de coeficientes $\mathbb{\beta} =$`r beta_pgd`.
  Optamos por generar todas las simulaciones una única vez y tomar submuestras 
