...
sintesis %>%
  mutate (
    ...
    prop_cubre = cell_spec(prop_cubre,"html",
                           color = ifelse(prop_cubre < 0.89,"red","blue"),
                           background = ifelse(prop_cubre < 0.86,"gray","white")
                           )
  ) %>%
  spread(n,prop_cubre) %>%
  kable(format="html", escape = F, label="Proporción Cubierta por los intervalos, por tamaño n.") %>%
...
### Gráficos de las Proporciones de no Cubrimiento por Distribución
...
#### Proporción de Parámetros no Cubiertos para el Error Exponencial


```{r intervalos exponenciales que cubren}
intervalos %>%
  filter(distr_eps=='exponencial',cubre==FALSE, n_sim <= 1000) %>%
  ggplot(aes(x = met_int, y=..count../sum(..count..) , fill=n, color = met_int)) +
    geom_bar() +
    facet_grid(fun_a~n)
```

#### Proporción de Parámetros no Cubiertos para el Error Lognormal

```{r intervalos lognormales que cubren}
intervalos %>%
  filter(distr_eps=='lognormal',cubre==FALSE, n_sim <= 1000) %>%
  ggplot(aes(x = met_int, y=..count../sum(..count..) , fill=n, color = met_int)) +
    geom_bar() +
    facet_grid(fun_a~n)
```

#### Proporción de Parámetros no Cubiertos para el Error Uniforme

```{r intervalos uniformes que cubren}
intervalos %>%
  filter(distr_eps=='uniforme',cubre==FALSE, n_sim <= 1000) %>%
  ggplot(aes(x = met_int, y=..count../sum(..count..) , fill=n, color = met_int)) +
    geom_bar() +
    facet_grid(fun_a~n)
```

#### Proporción de Parámetros no Cubiertos para el Error Chi Cuadrado

```{r intervalos chi que cubren}
intervalos %>%
  filter(distr_eps=='chi_cuadrado',cubre==FALSE, n_sim <= 1000) %>%
  ggplot(aes(x = met_int, y=..count../sum(..count..) , fill=n, color = met_int)) +
    geom_bar() +
    facet_grid(fun_a~n)
```

#### Proporción de Parámetros no Cubiertos para el Error T 

```{r intervalos T que cubren}
intervalos %>%
  filter(distr_eps=='student3',cubre==FALSE, n_sim <= 1000) %>%
  ggplot(aes(x = met_int, y=..count../sum(..count..) , fill=n, color = met_int)) +
    geom_bar() +
    facet_grid(fun_a~n)
```

#### Proporción de Parámetros no Cubiertos para el Cauchy

```{r intervalos cauchy que cubren}
intervalos %>%
  filter(distr_eps=='student1',cubre==FALSE, n_sim <= 1000) %>%
  ggplot(aes(x = met_int, y=..count../sum(..count..) , fill=n, color = met_int)) +
    geom_bar() +
    facet_grid(fun_a~n)
```
...
### Algunas Observaciones Sobre Estos Patrones
  ...
  Los incrementos tanto en el tamaño de muestra como en el número de repeticiones brindan en principio un incremento en el apego al valor de $\alfa$ para los intervalos asintóticos. Después de una cantidad de valores vemos que ambos se comportan en forma prácticamente indistinguible aunque es notable la precisión inicial del intervalo exacto. 
  Es imposible incrementar nuestro apego en la tasa de acierto al alfa más allá de cierto nivel, de hecho en algunos casos observamos regresiones al incrementar el valor de $B$. 
  Creemos que estas regresiones se deben a que en realidad el valor de esta proporción tiene un compotamiento oscilatorio y no realmente convergente respecto a $\alfa$. Probablemente para hacer un estudio completo de esta cuestión sea importante considerar también la longitud de los intervalos generados.  
...
intervalos %>%
  ...
  ggplot() + aes(sample =estimador, color = distr_eps) + geom_qq() + stat_qq_line() + facet_grid(n~.)
```
...
intervalos %>%
  filter(n_sim <= 50, is.element(n,c(10,100,1000,3000)),fun_a=='beta4',distr_eps == 'exponencial')  %>%
  ggplot() + aes(sample =estimador, color = distr_eps) + geom_qq() + stat_qq_line() + facet_grid(n~., scales="free")
```
...
intervalos %>%
  ...
  ggplot() + aes(sample =estimador, color = distr_eps) + geom_qq() + stat_qq_line() + facet_grid(n~distr_eps)
```
...
# Conclusiones
  ...
  * En casi todas al incrementar $n$ logramos llevar el nivel de los intervalos asintóticos prácticamente al de los exactos. 
  * El único caso donde no pudimos lograr esto fue en la distribución de tipo $Cauchy$, $t_1$. Esto era esperable dado que esta distribución no tiene esperanza y por tanto no cumple las hipótesis del teorema del límite central. 
